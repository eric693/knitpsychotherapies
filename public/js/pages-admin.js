...
App.page('users', {
  ...
  help: [
    ...
    '心理師的選單不會出現總覽、線上預約申請、候補遞補、今日看板、晤談提醒、報酬與扣繳（登入後直接進「我的工作台」）；管理者則不顯示今日看板。這是依角色決定的，勾不勾模組都一樣。',
  ...
  async render(el) {
    ...
    const groups = [
      ...
      ['報酬與扣繳', [['withholding_rate', '執行業務所得扣繳率（0-1）'], ['withholding_min', '所得稅起扣金額'],
        ...
        ['payout_slip_title', '勞務報酬單 標題'], ['payout_slip_note', '勞務報酬單 頁尾說明']]],
      ['證明書', [['cert_prefix', '證明書編號前綴'], ['center_director_license', '負責心理師證書字號（如 心理字1923號）'],
        ...
        ['cert_referral_clinic_title', '轉介單（一式三聯）標題'],
        ['referral_clinic_copies', '轉介單的聯別（逗號分隔）'],
        ['referral_clinic_targets', '轉介單的建議轉介機構（每行一家）'],
        ['referral_reply_options', '轉介回覆選項（逗號分隔，醫療端勾選）'],
        ['cert_profile_minor_title', '未成年個案基本資料表 標題'],
